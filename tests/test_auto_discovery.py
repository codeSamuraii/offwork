...
from offwork.graph.analyzer import find_bare_calls
...
def test_auto_discover_no_warning_for_local_variables(tmp_path: Path) -> None:
    ...
    offwork_warnings = [
        w for w in caught
        if "offwork" in str(w.filename) and "callback" in str(w.message)
    ]
    assert offwork_warnings == []
...
def test_auto_discover_no_warning_for_closure_vars(tmp_path: Path) -> None:
    ...
    offwork_warnings = [
        w for w in caught
        if "offwork" in str(w.filename) and "fn" in str(w.message)
    ]
    assert offwork_warnings == []
