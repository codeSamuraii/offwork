...
from offwork.__main__ import (
    _detect_offwork_extras,
    _detect_script_packages,
    main,
)
from offwork._venv import (
    TempVenv,
    _DEFAULT_PREFIX,
    _find_project_root,
    _venv_python,
...
class TestFindProjectRoot:
    ...
    def test_root_contains_offwork(self) -> None:
        root = _find_project_root()
        assert root is not None
        assert (root / "offwork").is_dir()


class TestTempVenv:
    @pytest.mark.asyncio
    async def test_creates_and_cleans_up(self) -> None:
        async with temp_venv(install_offwork=False) as venv:
    ...
    @pytest.mark.asyncio
    async def test_installs_offwork(self) -> None:
        async with temp_venv(install_offwork=True) as venv:
            result = subprocess.run(
                [str(venv.python), "-c", "import offwork; print(offwork.serialize)"],
                capture_output=True,
                text=True,
            )
            assert result.returncode == 0
...
class TestDetectScriptPackages:
    ...
    def test_skips_local_package_reachable_via_cwd(
        ...
        Mirrors ``offwork run examples/foo.py`` invoked from the project root,
    ...
    def test_install_package_as_attribute_form(
        ...
        """``with offwork.install_package_as(...)`` is also recognized."""
        script = tmp_path / "test_ipa_attr.py"  # type: ignore[operator]
        script.write_text(
            "import offwork\n"
            'with offwork.install_package_as("python-multipart"):\n'
            "    import multipart\n"
        )
        packages = _detect_script_packages(str(script))
