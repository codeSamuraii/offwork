...
away.connect("redis://localhost:6379")  # configure backend (sync)
await away.serve("redis://...", concurrency=4)  # start worker loop
...
away.progress(75.0)                     # report percentage
away.progress(3, 10, message="step 3")  # report current/total

# Serialization (sync)
away.serialize(func)                    # -> JSON string
away.reconstruct(json_str, "name")      # -> Python source string
away.pack(func, *args)                  # -> Task
await away.execute(task)                # -> return value

# Inspection
away.get_graph().to_mermaid(func)       # -> Mermaid diagram string
```
...
### Worker side: `await serve()` / `away worker`
...
- `away:tasks` -- task queue
- `away:result:{task_id}` -- per-task result (TTL: 300s)
- `away:heartbeat:{task_id}` -- worker heartbeat timestamp (TTL: 30s)
- `away:cancel:{task_id}` -- cancellation flag (TTL: 3600s)
- `away:progress:{task_id}` -- latest progress JSON (TTL: 300s)
- `away:schedule:{schedule_id}` -- schedule cancellation flag (TTL: 30 days)
- `away:throttle:{function_name}` -- throttle cooldown (TTL: throttle seconds)
- `away:notify` -- Pub/Sub channel for result notifications
...
Uses `aio-pika` (async AMQP 0-9-1). Tasks go through a single durable queue (`away.tasks`). Per-task results, heartbeats, cancellation flags, and progress live in single-message queues (`x-max-length: 1`) used as key-value slots. Result notifications fan out via a topic exchange (`away.notify`). Throttle queues use a fixed TTL with the cooldown expiry encoded in the message body.

URL scheme: `amqp://` or `amqps://` (e.g. `amqp://guest:guest@localhost/`). The `aio-pika` package is an optional dependency installed via `pip install away[rabbitmq]`.
...
`inspect.getsource()` retrieves the function's source. `textwrap.dedent()` normalizes indentation. `@trace` decorator lines are stripped so reconstructed code doesn't depend on away.
...
When a traced function calls an untraced user-defined function, away automatically discovers and registers it. This is recursive: if `traced_func()` calls `helper_a()` which calls `helper_b()`, all three end up in the graph.
...
If the function captures variables from an enclosing scope, away uses a multi-tier capture strategy:
...
- `__away_traced__ = True` -- marker attribute
...
|---------|---------------------|
| Redis | `SET away:cancel:{task_id} 1 EX 3600` |
| Local | In-memory `set()` in the broker |

## Progress reporting

Tasks can report progress via `away.progress(percent)` or `away.progress(current, total)`.
...
|---------|-----------------|
| Redis | `SET away:progress:{task_id} <json> EX 300` |
...
A lightweight **guest agent** (`guest_agent.py`) runs inside the container. It is stdlib-only (no away install required) and communicates with the worker over TCP using a **length-prefixed JSON protocol** (4-byte big-endian header + UTF-8 JSON payload).
...
Environment variables `AWAY_SANDBOX_DOCKER_IMAGE` and `AWAY_SANDBOX_DOCKER_CONTAINER` override the image and container names.
...
away worker --backend redis://localhost:6379 -c 4
away worker --backend redis://localhost:6379 --no-auto-install
away worker --backend redis://localhost:6379 --tmp              # isolated temp venv
away worker --backend redis://localhost:6379 --sandbox          # Docker sandbox
away worker --backend redis://localhost:6379 --require-signing  # reject unsigned tasks
away worker --backend redis://localhost:6379 --pair             # pair then serve

# Signing — token-based
away token generate           # write ~/.away/token
away token show               # display current token source
away token clear              # remove ~/.away/token

# Signing — PIN-based pairing
away pair --backend URL                   # client side: enter PIN displayed by worker
away pair --backend URL --role worker     # worker side (or use `away worker --pair`)
away pair --backend URL --clear           # remove ~/.away/{client,worker}.key

# Sandbox management
away sandbox setup       # build Docker sandbox image
away sandbox status      # show Docker sandbox status
away sandbox teardown    # remove Docker sandbox

# Run a script in a temporary venv (auto-detects and installs dependencies)
away run examples/script.py

# Show configuration
away info

# Serialize a function to JSON
away serialize mymodule:csv_to_json

# Reconstruct source from a graph file
away reconstruct graph.json csv_to_json
```
