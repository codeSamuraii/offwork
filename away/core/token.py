...
protocol (see :mod:`away.core.pairing`).  A token is a random
...
1. ``AWAY_SIGNING_TOKEN`` environment variable (hex-encoded)
2. ``~/.away/token`` file (hex-encoded)
3. ``~/.away/{client,worker}.key`` file (raw bytes, from pairing)
...
_TOKEN_ENV_VAR = "AWAY_SIGNING_TOKEN"

# File persistence
_DEFAULT_KEY_DIR = Path.home() / ".away"
_TOKEN_FILE = "token"
...
def save_token(
    ...
    """Persist a hex-encoded token to ``~/.away/token``.
    ...
    key_dir
        Override the default ``~/.away`` directory.
...
def load_token(key_dir: Path | None = None) -> str | None:
    ...
    1. ``AWAY_SIGNING_TOKEN`` environment variable
    2. ``~/.away/token`` file
