"""Temporary virtual environment management for away."""
...
def cleanup_stale_venvs(
    ...
    """Remove stale away temp directories left behind by crashed processes.
...
async def temp_venv(
    *,
    install_away: bool = True,
    extras: Sequence[str] = (),
    prefix: str = _DEFAULT_PREFIX,
) -> AsyncIterator[TempVenv]:
    """Create a temporary venv, optionally install away, yield, then clean up.
    ...
    ----------
    install_away:
        Install away into the venv (editable from source tree, or from PyPI).
    extras:
    ...
    try:
        ...
        if install_away:
            ...
            if extras:
                spec += f"[{','.join(extras)}]"
            print("Installing away into temporary venv...", file=sys.stderr)
