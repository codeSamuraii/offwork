...
Runs the guest agent (:mod:`~away.worker.sandbox.guest_agent`) inside
...
first use, so ``away sandbox setup`` is optional (but recommended in
...
from away.core.task import _resolve, _to_jsonable
from away.core.progress import _progress_callback
from away.worker.sandbox._protocol import async_recv, async_send
...
_DEFAULT_IMAGE = f"away-sandbox:{_assets_digest()}"
_DEFAULT_CONTAINER = "away-sandbox"
...
class DockerSandbox:
    ...
    async def _read_response(
        ...
        to *progress_cb* so that ``away.progress()`` calls inside the
