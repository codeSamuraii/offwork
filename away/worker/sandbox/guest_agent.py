#!/usr/bin/env python3
"""Lightweight guest agent for away sandbox.
...
identical to ``away.worker.sandbox._protocol``.
...
_OBJECT_SENTINEL = "__away_obj__"
_BYTES_SENTINEL = "__away_bytes__"
_BUILTIN_SENTINEL = "__away_builtin__"
_TUPLE_SENTINEL = "__away_tuple__"
_DICT_SENTINEL = "__away_dict__"
_PICKLE_SENTINEL = "__away_pickle__"
...
# ---------------------------------------------------------------------------
# Sentinel encoding / decoding (mirrors away.core.task)
# ---------------------------------------------------------------------------
...
def _install_away_shim(
    writer: asyncio.StreamWriter | None,
) -> tuple[Any, ...]:
    """Install a fake ``away`` package so ``from away import progress`` works.
    ...
    # Build a minimal away package hierarchy.
    fake = types.ModuleType("away")
    fake.progress = _progress  # type: ignore[attr-defined]
    fake_core = types.ModuleType("away.core")
    fake_core_progress = types.ModuleType("away.core.progress")
    ...
    saved = (
        sys.modules.get("away"),
        sys.modules.get("away.core"),
        sys.modules.get("away.core.progress"),
    )
    sys.modules["away"] = fake
    sys.modules["away.core"] = fake_core
    sys.modules["away.core.progress"] = fake_core_progress
    return saved


def _uninstall_away_shim(saved: tuple[Any, ...]) -> None:
    for key, prev in zip(
        ("away", "away.core", "away.core.progress"), saved
    ):
        if prev is None:
...
async def _execute_request(
    ...
    try:
        ...
        # Compile and exec
        code = compile(source, f"<away-sandbox:{function_name}>", "exec")
        namespace: dict[str, Any] = {}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="away sandbox guest agent")
