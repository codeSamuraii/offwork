...
from away.core.progress import _progress_callback
...
def _create_backend(url: str, **kwargs: Any) -> Backend:
    ...
    if scheme in ("redis", "rediss"):
        from away.worker.backends.redis import RedisBackend
...
def connect(url: str | None = None, **kwargs: Any) -> Backend:
    ...
    url
        ...
        When *None*, the ``AWAY_BACKEND`` environment variable is used.
...
def get_backend() -> Backend:
    ...
    ``AWAY_BACKEND`` environment variable is checked and used
    to auto-connect.
    """
    global _active_backend
    if _active_backend is None:
        ...
        if env_url:
            ...
            raise RuntimeError(
                "No backend connected. Call away.connect('redis://...') or away.connect('https://...') "
...
async def submit_remote(
    ...
    **kwargs: Any,
    ...
    """
    from away.graph.graph import Graph  # circular
    ...
    opts = getattr(wrapper, "__away_options__", {})
    task = Task(
...
async def serve(
    ...
    *,
    ...
    sandbox
        ``True`` or a :class:`~away.worker.sandbox.DockerSandbox`
    ...
    require_signing
        ...
        ``~/.away/worker.key`` (written by ``away pair``).
    """
    from away.worker.sandbox import DockerSandbox
    ...
    logger.info(
        "away worker v%s  \u2502  %s  \u2502  concurrency=%d  \u2502  "
    ...
    if require_signing:
        ...
        if signing_key is None:
            logger.error(
                ...
                "Set AWAY_SIGNING_TOKEN, run 'away token generate', "
                "or run 'away pair' to pair with a client."
            )
            sys.exit(1)
