...
from away import serialize, reconstruct
from away._venv import temp_venv
from away.worker.deps import DEFAULT_IMPORT_TO_PACKAGE
from away.worker.remote import serve
from away.graph.analyzer import _parse_install_package_as, _parse_worker_only_import


def _build_worker_cmd(python: str, args: argparse.Namespace) -> list[str]:
    """Rebuild the worker CLI command for re-exec, without --tmp."""
    cmd = [python, "-m", "away", "worker", "--backend", args.backend]
...
async def _run_in_tmp_venv(args: argparse.Namespace) -> None:
    ...
    async with temp_venv(install_away=True, extras=extras) as venv:
...
def _configure_logging(level: int) -> None:
    """Set up away logger with a stderr handler."""
...
def _cmd_worker(args: argparse.Namespace) -> None:
    if not args.backend:
        print("Error: --backend is required (or set AWAY_BACKEND).", file=sys.stderr)
        sys.exit(1)
...
async def _pair_then_serve(args: argparse.Namespace) -> None:
    ...
    from away.core.pairing import generate_pin, save_shared_key, initiate_pairing
    from away.worker.remote import connect, disconnect
    ...
    print(f"    away pair --backend {args.backend}")
    print("\n  Waiting for client...\n")
    ...
    print(f"  ✓ Paired successfully. Key saved to ~/.away/worker.key")
...
def _cmd_info(_args: argparse.Namespace) -> None:
    try:
        ver = pkg_version("away")
    except Exception:
        ver = "unknown"

    print(f"away {ver}")
    print(f"  AWAY_BACKEND = {os.environ.get('AWAY_BACKEND', '(not set)')}")
...
def _detect_script_packages(script: str) -> list[str]:
    ...
    for m, explicit_package in sorted(modules.items()):
        if m in sys.stdlib_module_names or m == "away":
            continue
        if m in skip:
            continue
...
def _detect_away_extras(script: str) -> list[str]:
    """Detect away extras needed by a script (e.g. redis from connect/serve calls)."""
    _source, tree = _parse_script(script)
    if tree is None:
        return []
...
def _collect_extras(args: argparse.Namespace, script: Path) -> list[str]:
    """Gather away extras and third-party packages for a script."""
    extras: list[str] = list(args.extra or [])
    backend = os.environ.get("AWAY_BACKEND", "")
    if backend.startswith(("redis://", "rediss://")) and "redis" not in extras:
        extras.append("redis")
    for extra in _detect_away_extras(str(script)):
...
def _add_worker_parser(sub: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    p = sub.add_parser("worker", help="Start a away worker")
    p.add_argument(
        "--backend",
        default=os.environ.get("AWAY_BACKEND"),
        help="Backend URL, e.g. redis://localhost:6379 (default: $AWAY_BACKEND)",
    )
    p.add_argument(
...
def _add_run_parser(sub: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    p = sub.add_parser(
        "run", help="Run a Python script in a temporary venv with away installed",
    )
...
def _cmd_sandbox(args: argparse.Namespace) -> None:
    """Handle ``away sandbox setup|status|teardown`` subcommands."""
    action = getattr(args, "sandbox_action", None)
    if action is None:
        print("Usage: away sandbox {setup|status|teardown}", file=sys.stderr)
        sys.exit(1)
...
async def _sandbox_status() -> None:
    ...
    if shutil.which("docker") is not None:
        from away.worker.sandbox.docker import (
            _image_exists,
            _container_exists,
            _container_running,
        )
        image = os.environ.get("AWAY_SANDBOX_DOCKER_IMAGE", "away-sandbox")
        container = os.environ.get("AWAY_SANDBOX_DOCKER_CONTAINER", "away-sandbox")
        ...
        if not img_ok:
            print("  Hint: run 'away sandbox setup' to build the image")
    else:
...
async def _docker_setup() -> None:
    ...
    from away.worker.sandbox.docker import _build_image, _image_exists
    image = os.environ.get("AWAY_SANDBOX_DOCKER_IMAGE", "away-sandbox")
    ...
    print(f"  away worker --backend redis://localhost:6379 --sandbox")
...
def _cmd_pair(args: argparse.Namespace) -> None:
    """Handle ``away pair`` — client-side PIN-based key exchange."""
    from away.core.pairing import load_shared_key
    ...
    if existing is not None and not args.force:
        print(
            ...
            "Use --force to overwrite it, or 'away pair --clear' to remove it.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _cmd_pair_clear(args: argparse.Namespace) -> None:
    """Handle ``away pair --clear``."""
    from away.core.pairing import clear_shared_key
...
async def _pair_async(args: argparse.Namespace, role: str) -> None:
    ...
    try:
        ...
        print(f"    Key saved to ~/.away/{role}.key\n")
    finally:
        await disconnect()
...
def _add_pair_parser(sub: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    ...
    p.add_argument(
        "--backend",
        default=os.environ.get("AWAY_BACKEND"),
        help="Backend URL for the pairing channel (default: $AWAY_BACKEND)",
    )
    p.add_argument(
        "--role", default="client", choices=("client", "worker"),
        help="Role of this machine in the pairing (default: client). "
             "Use 'away worker --pair' instead of '--role worker'.",
    )
    p.add_argument(
        "--pin", default=None,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="away", description="away - distributed task execution",
    )
    sub = parser.add_subparsers(dest="command")
    _add_worker_parser(sub)
    _add_run_parser(sub)
    sub.add_parser("info", help="Show away configuration")
...
def _dispatch_pair(args: argparse.Namespace) -> None:
    """Route ``away pair`` to clear or pairing handler."""
    if args.clear:
...
def _cmd_token(args: argparse.Namespace) -> None:
    """Handle ``away token`` subcommands."""
    action = getattr(args, "token_action", None)
    if action is None:
        print("Usage: away token {generate|show|clear}", file=sys.stderr)
        sys.exit(1)
...
def _cmd_token_generate(args: argparse.Namespace) -> None:
    ...
    from away.core.token import generate_token, save_token, load_token

    existing = load_token()
    if existing is not None and not args.force:
        print(
            "A signing token already exists.\n"
            "Use --force to overwrite it, or 'away token clear' to remove it.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(f"    export AWAY_SIGNING_TOKEN={token_hex}\n")
    print("  Or start the worker with signing enabled:")
    print("    away worker --backend redis://localhost:6379 --require-signing\n")


def _cmd_token_show(_args: argparse.Namespace) -> None:
    """Show the current signing token status."""
    from away.core.token import load_token, _TOKEN_ENV_VAR
