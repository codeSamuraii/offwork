...
protocol (see :mod:`pyfuse.core.pairing`).  A token is a random
...
1. ``PYFUSE_SIGNING_TOKEN`` environment variable (hex-encoded)
2. ``~/.pyfuse/token`` file (hex-encoded)
3. ``~/.pyfuse/{client,worker}.key`` file (raw bytes, from pairing)
...
_TOKEN_ENV_VAR = "PYFUSE_SIGNING_TOKEN"
...
def save_token(
    ...
    """Persist a hex-encoded token to ``~/.pyfuse/token``.
...
def load_token(key_dir: Path | None = None) -> str | None:
    ...
    1. ``PYFUSE_SIGNING_TOKEN`` environment variable
    2. ``~/.pyfuse/token`` file
...
def resolve_signing_key(role: str, key_dir: Path | None = None) -> bytes | None:
    ...
    1. ``PYFUSE_SIGNING_TOKEN`` environment variable
    2. ``~/.pyfuse/token`` file
    3. ``~/.pyfuse/{role}.key`` (from pairing)
