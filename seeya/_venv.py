"""Temporary virtual environment management for seeya."""
...
def cleanup_stale_venvs(
    ...
    """Remove stale seeya temp directories left behind by crashed processes.
...
async def temp_venv(
    *,
    install_seeya: bool = True,
    extras: Sequence[str] = (),
    prefix: str = _DEFAULT_PREFIX,
) -> AsyncIterator[TempVenv]:
    """Create a temporary venv, optionally install seeya, yield, then clean up.
    ...
    ----------
    install_seeya:
        Install seeya into the venv (editable from source tree, or from PyPI).
    extras:
    ...
    try:
        ...
        if install_seeya:
            ...
            if extras:
                spec += f"[{','.join(extras)}]"
            print("Installing seeya into temporary venv...", file=sys.stderr)
