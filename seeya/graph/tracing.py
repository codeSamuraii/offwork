...
from seeya.typing import TracedFunction
...
def _make_start_method(
    ...
    async def start(*args: Any, backend: str | Backend | None = None, **kwargs: Any) -> object:
        from seeya.worker.remote import submit_remote  # circular
...
def _make_start_at_method(
    ...
    async def start_at(dt: Any, *args: Any, backend: str | Backend | None = None, **kwargs: Any) -> object:
        from seeya.worker.remote import submit_remote_scheduled  # circular
...
def _make_run_every_method(
    ...
    async def run_every(
        ...
        **kwargs: Any,
        ...
        from seeya.worker.remote import submit_recurring  # circular
...
def _attach_traced_attrs(
    ...
    """Attach seeya metadata and .start()/.run()/.map() to a traced wrapper."""
    wrapper.__seeya_traced__ = True  # type: ignore[attr-defined]
...
# Derive our own top-level package name so we never inline seeya internals.
