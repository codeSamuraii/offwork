...
protocol (see :mod:`seeya.core.pairing`).  A token is a random
...
1. ``SEEYA_SIGNING_TOKEN`` environment variable (hex-encoded)
2. ``~/.seeya/token`` file (hex-encoded)
3. ``~/.seeya/{client,worker}.key`` file (raw bytes, from pairing)
...
_TOKEN_ENV_VAR = "SEEYA_SIGNING_TOKEN"

# File persistence
_DEFAULT_KEY_DIR = Path.home() / ".seeya"
_TOKEN_FILE = "token"
...
def save_token(
    ...
    """Persist a hex-encoded token to ``~/.seeya/token``.
    ...
    key_dir
        Override the default ``~/.seeya`` directory.
...
def load_token(key_dir: Path | None = None) -> str | None:
    ...
    1. ``SEEYA_SIGNING_TOKEN`` environment variable
    2. ``~/.seeya/token`` file
