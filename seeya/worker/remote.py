...
from seeya.core.progress import _progress_callback
...
def _create_backend(url: str, **kwargs: Any) -> Backend:
    ...
    if scheme in ("redis", "rediss"):
        from seeya.worker.backends.redis import RedisBackend
...
def connect(url: str | None = None, **kwargs: Any) -> Backend:
    ...
    url
        ...
        When *None*, the ``SEEYA_BACKEND`` environment variable is used.
...
def get_backend() -> Backend:
    ...
    ``SEEYA_BACKEND`` environment variable is checked and used
    to auto-connect.
    """
    global _active_backend
    if _active_backend is None:
        ...
        if env_url:
            ...
            raise RuntimeError(
                "No backend connected. Call seeya.connect('redis://...') or seeya.connect('https://...') "
...
async def submit_remote(
    ...
    **kwargs: Any,
    ...
    """
    from seeya.graph.graph import Graph  # circular
    ...
    opts = getattr(wrapper, "__seeya_options__", {})
    task = Task(
...
async def serve(
    ...
    *,
    ...
    sandbox
        ``True`` or a :class:`~seeya.worker.sandbox.DockerSandbox`
    ...
    require_signing
        ...
        ``~/.seeya/worker.key`` (written by ``seeya pair``).
    """
    from seeya.worker.sandbox import DockerSandbox
    ...
    logger.info(
        "seeya worker v%s  \u2502  %s  \u2502  concurrency=%d  \u2502  "
    ...
    if require_signing:
        ...
        if signing_key is None:
            logger.error(
                ...
                "Set SEEYA_SIGNING_TOKEN, run 'seeya token generate', "
                "or run 'seeya pair' to pair with a client."
            )
            sys.exit(1)
