...
Runs the guest agent (:mod:`~seeya.worker.sandbox.guest_agent`) inside
...
first use, so ``seeya sandbox setup`` is optional (but recommended in
...
from seeya.core.task import _resolve, _to_jsonable
from seeya.core.progress import _progress_callback
from seeya.worker.sandbox._protocol import async_recv, async_send
...
_DEFAULT_IMAGE = f"seeya-sandbox:{_assets_digest()}"
_DEFAULT_CONTAINER = "seeya-sandbox"
...
class DockerSandbox:
    ...
    async def _read_response(
        ...
        to *progress_cb* so that ``seeya.progress()`` calls inside the
