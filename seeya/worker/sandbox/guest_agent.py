#!/usr/bin/env python3
"""Lightweight guest agent for seeya sandbox.
...
identical to ``seeya.worker.sandbox._protocol``.
...
_OBJECT_SENTINEL = "__seeya_obj__"
_BYTES_SENTINEL = "__seeya_bytes__"
_BUILTIN_SENTINEL = "__seeya_builtin__"
_TUPLE_SENTINEL = "__seeya_tuple__"
_DICT_SENTINEL = "__seeya_dict__"
_PICKLE_SENTINEL = "__seeya_pickle__"
...
# ---------------------------------------------------------------------------
# Sentinel encoding / decoding (mirrors seeya.core.task)
# ---------------------------------------------------------------------------
...
def _install_seeya_shim(
    writer: asyncio.StreamWriter | None,
) -> tuple[Any, ...]:
    """Install a fake ``seeya`` package so ``from seeya import progress`` works.
    ...
    # Build a minimal seeya package hierarchy.
    fake = types.ModuleType("seeya")
    fake.progress = _progress  # type: ignore[attr-defined]
    fake_core = types.ModuleType("seeya.core")
    fake_core_progress = types.ModuleType("seeya.core.progress")
    ...
    saved = (
        sys.modules.get("seeya"),
        sys.modules.get("seeya.core"),
        sys.modules.get("seeya.core.progress"),
    )
    sys.modules["seeya"] = fake
    sys.modules["seeya.core"] = fake_core
    sys.modules["seeya.core.progress"] = fake_core_progress
    return saved


def _uninstall_seeya_shim(saved: tuple[Any, ...]) -> None:
    for key, prev in zip(
        ("seeya", "seeya.core", "seeya.core.progress"), saved
    ):
        if prev is None:
...
async def _execute_request(
    ...
    try:
        ...
        # Compile and exec
        code = compile(source, f"<seeya-sandbox:{function_name}>", "exec")
        namespace: dict[str, Any] = {}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="seeya sandbox guest agent")
